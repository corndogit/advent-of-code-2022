# Golfed solution:
with open("../inputs/day1.txt") as file: meals = list(map(lambda x: list(map(lambda y: int(y if len(y) > 0 else 0), x.split('\n'))), file.read().split('\n\n')))
print("Part 1: {}\nPart 2: {}".format(max(map(sum, [m for m in meals])), sum(sorted(map(sum, [m for m in meals]), reverse=True)[:3])))
